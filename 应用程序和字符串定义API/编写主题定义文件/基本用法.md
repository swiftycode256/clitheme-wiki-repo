...
虽然CLItheme在文件扩展名上没有要求，但是最好将文件的扩展名定义为`.clithemedef.txt`。这样子你可以更好的辨识文件，并且使用`.txt`会确保该文件会被系统使用正确的应用程序（如文本编辑器）打开。
...
如需了解关于子路径的更多信息，请参考[**应用程序和字符串定义API/应用程序适配/路径名称和数据结构**](../../应用程序和字符串定义API/应用程序适配/1.%20路径名称和数据结构)。
