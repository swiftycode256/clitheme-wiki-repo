...
本文章将介绍[主题定义文件](../附录：主题定义文件通用语法/文件格式和语法)中的`manpage`段落语法。这个段落允许你添加自定义的Unix文档手册（`manpage`）文档，达到文档自定义的目的。
...
在定义文件中，所有的Unix文档手册都是在`manpages`段落中实现的。请使用以下语法定义`manpages`段落：
...
你可以使用`<include_file>`语法来使用定义文件所在文件夹中的一个文件来作为文档手册的文件内容，并且在下一行中使用`as:`语法来定义目标文件路径。在`<include_file>`后根据**目标文件路径**语法添加引用文件的相对文件路径（根据**定义文件所在的文件夹/路径**计算，不是终端的PWD/当前路径位置），并且在`as:`后添加目标文件路径。
...
主题定义文件写好后，你需要应用这个文件到系统中。请在终端执行`clitheme apply-theme <文件名>`以应用主题。应用好主题后，请使用`clitheme-man`来使用这些文档手册。详见[使用clitheme-man](2.%20使用clitheme-man)。