...
你可以在主题定义文件中定义和使用**内容变量**，可以帮助你面去重复复制粘贴重复内容，并且可以更轻松的批量更改内容。

## 定义变量

你可以在段落之外或在段落内定义变量。如需定义变量，请使用`setvar[<变量名称>]: <内容>`语法。

你也可以同时指定多个由空格分开的变量名称，如`setvar[var1 var2]:`。
...
> [!IMPORTANT]
> 请注意，变量名称不能包含`(){}[]`中的任一字符，并且不能为`ESC`（因为会和`substesc`功能选项冲突）。
...
如需使用变量，请先[启用](./可以设定的选项.md#如何设定选项)`substvar`选项（使用`set_options substvar`），然后在内容中使用`{{<变量名称>}}`语法，比如`{{example_var}}`。
...
你也可以在指定**系统语言**和**选项设定**的地方使用内容变量，比如：
...
{substrules}
    ...
    [subst_regex] Some content
        ...
        [locale] {{langs}}
            Substitute content
        [/locale]
    # 将会设定strictcmdmatch和endmatchhere选项
    [/subst_regex] {{opts}}
{/substrules}
```
