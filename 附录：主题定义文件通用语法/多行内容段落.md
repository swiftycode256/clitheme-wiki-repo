...
- `header`：[header段落](header段落)
- `substrules`：[命令行输出替换/编写定义文件](../命令行输出替换/1.%20编写定义文件)
- `entries`：[应用程序和字符串定义API/编写主题定义文件/多行内容段落](../应用程序和字符串定义API/编写主题定义文件/多行内容段落)
- `manpages`： [自定义manpage文档/编写定义文件](../自定义manpage文档/1.%20编写定义文件)
...
你也可以使用`set_options`来对之后定义的所有段落应用这些选项，如`set_options leadtabs:1`或`set_options leadspaces:4`。详细请见[可以设定的选项](./可以设定的选项)。
