...
- **If the output contains any terminal control sequences, they must be included in the match expression.** You can use the `clitheme-exec --showchars` command to see these control characters in green color. (see [Using clitheme-exec](2.%20Using%20clitheme-exec.md) for more information)
    - To use the ANSI Escape character (`{{ESC}}`), please [enable](../[Appendix]%20Common%20syntax%20of%20theme%20definition%20files/Available%20options.md#How%20to%20set%20options) the `substesc` option and use `{{ESC}}` in content
    - To use other characters like `\x08` or `\x07`, please [enable](../[Appendix]%20Common%20syntax%20of%20theme%20definition%20files/Available%20options.md#How%20to%20set%20options) the `substchar` option and use phrases like `{{[x08]}}` in content
...
You can choose [other matching mechanisms](../[Appendix]%20Common%20syntax%20of%20theme%20definition%20files/Available%20options.md#Command%20filters) by [setting options](../[Appendix]%20Common%20syntax%20of%20theme%20definition%20files/Available%20options.md#How%20to%20set%20options) for command filters, such as:
