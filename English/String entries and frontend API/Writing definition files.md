...
This article will detail how to define string entries in a [theme definition file](../[Appendix]%20Common%20syntax%20of%20theme%20definition%20files/File%20format%20and%20syntax.md).
...
> [!TIP]
> Please see [Available options](../[Appendix]%20Common%20syntax%20of%20theme%20definition%20files/Available%20options.md) for more options and features like line boundaries and content variables.
...
> Please see [Multi-line content block](../[Appendix]%20Common%20syntax%20of%20theme%20definition%20files/Multi-line%20content%20block.md) for how indents are processed and other information.

### Using `<in_domainapp>` and `<in_subsection>`

You can use `<in_domainapp>` and `<in_subsection>` to reduce repetition in [path names](./Developer%20docs/1.%20Path%20names%20and%20data%20structure.md). After defining these parameters, they will be added in front of all path names in `[entry]` for subsequent string entry definitions, with the format of `<domain> <app> <subsections> [Path]`.
